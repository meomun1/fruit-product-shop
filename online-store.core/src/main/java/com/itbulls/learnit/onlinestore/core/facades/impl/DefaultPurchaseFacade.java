...
public class DefaultPurchaseFacade implements PurchaseFacade{
    ...
    @Override
    public void createPurchase(User attribute, Product product) { // save id, customer, products, purchaseStatus
    ...
    public List<Purchase> getNotCompletedPurchases() {
        return purchaseConverter.convertPurchaseDtosToPurchases(purchaseDao.getNotCompletedPurchase(LAST_STATUS_OF_ORDER_FULFILMENT_ID));
    }
    ...
    public void markFulfilmentStageForPurchaseIdAsCompleted(Integer purchaseId) {
        ...
        int purchaseStatusId = purchaseStatus.getId() + 1;
        purchaseStatus.setId(purchaseStatusId);
		purchase.setPurchaseStatus(purchaseStatus); 

        purchaseDao.updatePurchase(purchaseConverter.convertPurchaseToPurchaseDto(purchase));

        if(LAST_STATUS_OF_ORDER_FULFILMENT_ID.equals(purchaseStatusId) && purchase.getCustomer().getReferrerUser() != null){
